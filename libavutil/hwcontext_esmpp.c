...
#include "imgutils.h"
#include "mem.h"
#include "esmpp_utils.h"
#ifdef TRANSCODE_HOST
#include "es_host_query_cap.h"
#endif

#if !defined(LIBDRM_VERSION_NUMBER) || LIBDRM_VERSION_NUMBER < 0x02046400L
#define DRM_FORMAT_P010 fourcc_code('P', '0', '1', '0')
#define DRM_FORMAT_P210 fourcc_code('P', '2', '1', '0')
#endif


#define DEV_SELECT_MAX_LOOP 100
#define FIND_DEV_SLEEP_FOR_NEXT_LOCK 100000
static const struct {
...
static int esmpp_device_create(AVHWDeviceContext *hwdev, const char *device, AVDictionary *opts, int flags) {
    ...
    /* find candidate device, select the usage ratio lowest one, in this stage the resource will be ocupatied
        is unknown, the default value as below, can be changed manually through config*/
    int dec_ratio = 5, enc_ratio = 4, filter_ratio = 1;
    hwctx->device_idx = 0;
    hwctx->die_idx = 0;
    ...
    opt_d = av_dict_get(opts, "res_ratio", NULL, 0);
    if (opt_d) {
        if (sscanf(opt_d->value, "%d:%d:%d", &dec_ratio, &enc_ratio, &filter_ratio) == 3) {
            av_log(hwdev, AV_LOG_INFO, "configure res_ratio:%s, resource ratio, dec:%d,enc:%d,filter:%d\n",
                   opt_d->value, dec_ratio, enc_ratio, filter_ratio);
            int total = dec_ratio + enc_ratio + filter_ratio;
            if (total != 10) {
                dec_ratio = dec_ratio * 10 / total;
                enc_ratio = enc_ratio * 10 / total;
                filter_ratio = 10 - dec_ratio - enc_ratio;
                av_log(hwdev, AV_LOG_WARNING, "res_ratio:%s sum not 10, recalculate it dec:%d,enc:%d,filter:%d\n",
                   opt_d->value, dec_ratio, enc_ratio, filter_ratio);
            }
        } else {
            av_log(hwdev, AV_LOG_WARNING, "invalid resource_ratio %s, use default value. dec:%d,enc:%d,filter:%d\n",
                   opt_d->value, dec_ratio, enc_ratio, filter_ratio);
        }
    }

    opt_d = av_dict_get(opts, "device_idx", NULL, 0);
    if (opt_d) {
        long dev_idx = 0;
        dev_idx = strtol(opt_d->value, NULL, 10);
        hwctx->device_idx = dev_idx;
        if (hwctx->device_idx < -1) {
            av_log(hwdev, AV_LOG_ERROR, "%s set device_idx: %d invalid.\n", __func__, hwctx->device_idx);
            return -1;
        } else if (hwctx->device_idx == -1) {
#ifdef TRANSCODE_HOST
            int ret = -1;
            int loop_num = 0;
            while (true) {
                ret = find_candidate(dec_ratio, enc_ratio, filter_ratio, RPC_TIMEOUT_MS);
                if (ret < 0) {
                    av_log(hwdev, AV_LOG_ERROR, "%s auto select device_idx failed, ret %d.\n", __func__, ret);
                    hwctx->device_idx = 0;
                    break;
                }
                hwctx->device_idx = ret;
                ret = lock_dev(hwctx->device_idx, RPC_TIMEOUT_MS);
                if (ret == 1) {
                    av_log(hwdev, AV_LOG_DEBUG, "%s lock the dev %d, ret %d.\n", __func__, hwctx->device_idx, ret);
                    break;
                } else if (ret == 0) {
                    av_log(hwdev, AV_LOG_DEBUG, "%s can't lock the dev %d, ret %d. lock again, loop_num %d\n",
                           __func__, hwctx->device_idx, ret, loop_num);
                    usleep(FIND_DEV_SLEEP_FOR_NEXT_LOCK);
                    loop_num++;
                    if (loop_num < DEV_SELECT_MAX_LOOP)
                        continue;
                    else
                        break;
                } else {
                    av_log(hwdev, AV_LOG_ERROR, "%s Error: Can't lock the dev %d, ret %d.\n",
                           __func__, hwctx->device_idx, ret);
                    break;
                }
            }
        ...
        av_log(hwdev, AV_LOG_INFO, "%s set device_idx: %d.\n", __func__, hwctx->device_idx);
    }

    opt_d = av_dict_get(opts, "die_idx", NULL, 0);
    if (opt_d) {
        hwctx->die_idx = strtol(opt_d->value, NULL, 10);
        if (hwctx->die_idx < 0 || hwctx->die_idx > 1) {
            av_log(hwdev, AV_LOG_ERROR, "%s set die_idx: %d invalid.\n", __func__, hwctx->die_idx);
            return -1;
        }
        av_log(hwdev, AV_LOG_INFO, "%s set die_idx: %d.\n", __func__, hwctx->die_idx);
    }
...
static int esmpp_frames_get_constraints(AVHWDeviceContext *hwdev,
    ...
    ES_RETURN_VAL_IF_FAIL(constraints, AVERROR(EINVAL));
    int i;
...
static int esmpp_get_aligned_linesize(enum AVPixelFormat pix_fmt, int width, int plane) {
    ES_RETURN_VAL_IF_FAIL(width >= 0, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(plane >= 0, AVERROR(EINVAL));
...
static int esmpp_frames_init(AVHWFramesContext *hwfc) {
    ES_RETURN_VAL_IF_FAIL(hwfc, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(hwfc->device_ctx, AVERROR(EINVAL));
    AVESMPPFramesContext *avfc = hwfc->hwctx;
    ...
    buf_type = mpp_buffer_get_type(buf_type, hwctx->die_idx);
...
static void esmpp_frames_uninit(AVHWFramesContext *hwfc) {
    ES_RETURN_IF_FAIL(hwfc);
    AVESMPPFramesContext *avfc = hwfc->hwctx;
    ES_RETURN_IF_FAIL(avfc);
...
static int esmpp_get_buffer(AVHWFramesContext *hwfc, AVFrame *frame) {
    ES_RETURN_VAL_IF_FAIL(hwfc, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(frame, AVERROR(EINVAL));
...
static void esmpp_unmap_frame(AVHWFramesContext *hwfc, HWMapDescriptor *hwmap) {
    ES_RETURN_IF_FAIL(hwmap);
    ESMPPDRMMapping *map = hwmap->priv;
    ES_RETURN_IF_FAIL(map);
...
static int esmpp_map_frame(AVHWFramesContext *hwfc, AVFrame *dst, const AVFrame *src, int flags) {
    ES_RETURN_VAL_IF_FAIL(dst, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(src, AVERROR(EINVAL));
    const AVESMPPDRMFrameDescriptor *desc = (AVESMPPDRMFrameDescriptor *)src->data[0];
    ES_RETURN_VAL_IF_FAIL(desc, AVERROR(EINVAL));
    ESMPPDRMMapping *map;
    int err, i, p, plane;
    int mmap_prot;
    void *addr;
...
static int esmpp_transfer_get_formats(AVHWFramesContext *ctx,
    ...
    ES_RETURN_VAL_IF_FAIL(ctx, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(formats, AVERROR(EINVAL));
