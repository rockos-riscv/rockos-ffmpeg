...
typedef struct ESADECContext {
    ...
    char *buffer;
    int buffer_size;
    uint8_t stream[MAX_STREAM_LEN * MAX_CHANNELS];
    uint32_t offset;
...
static const AVChannelLayout layout[] = {AV_CHANNEL_LAYOUT_MONO,
...
static int get_stream_info(AVCodecContext *avctx, uint8_t *data, uint32_t size, es_frame_info *frame_info)
{
    ...
    if (frame_info->channels > MAX_CHANNELS || frame_info->channels == 0) {
        av_log(avctx, AV_LOG_ERROR, "unsupport channel number:%d\n", frame_info->channels);
        return AVERROR_UNKNOWN;
    }
    ...
    avctx->ch_layout = layout[frame_info->channels-1];
                                ...
                                int *got_frame_ptr, AVPacket *avpkt)
{
    ...
    if(frame->buf[0] && frame->buf[0]->size < size) {
        size = frame->buf[0]->size;
    }
