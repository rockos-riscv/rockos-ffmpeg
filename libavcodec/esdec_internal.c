...
static int esdec_output_buffer_fd_split(
    void *dwl_inst, void *dec_inst, int dev_mem_fd, ESOutputMemory *memory, int pp_count);
...
ESInputPort *esdec_allocate_input_port(ESDecCodec codec,
    ...
    for (int i = 0; i < input_buf_num; i++) {
        ...
        esdec_input_buffer_init(&input_buffer, mem->virtual_address, mem->bus_address, mem->logical_size, mem->size);
...
int esdec_allocate_all_output_memorys(
    ...
    for (int i = 0; i < port->mem_num; i++) {
        ...
        if (memory) {
            ...
            if (codec != ES_JPEG) {
                esdec_output_buffer_fd_split(dwl_inst, dec_inst, dev_mem_fd, memory, pp_count);
            } else {
                struct ESDecoderWrapper esdec;
                esdec.codec = DEC_JPEG;
                esdec.inst = dec_inst;
                esdec_output_buffer_fd_split(dwl_inst, &esdec, dev_mem_fd, memory, pp_count);
            }
...
static int esdec_allocate_more_output_memorys(
    ESDecCodec codec, ESOutputPort *port, ESVDecInst dec_inst, void *dwl_inst, int dev_mem_fd, int memory_count) {
    int ret = SUCCESS;
    int pp_count;
    int mem_num;
    ESOutputMemory *memory;
...
ESOutputPort *esdec_allocate_output_port(ESDecCodec codec,
    ...
    do {
        if (info.next_buf_size != 0) {
            ...
            ret = esdec_allocate_all_output_memorys(codec, port, dec_inst, dev_mem_fd, dwl_inst);
            if (ret == FAILURE) {
                break;
            }
...
int esdec_output_port_change(ESDecCodec codec, ESOutputPort *port, ESVDecInst dec_inst, int dev_mem_fd,int pp_count, int new_hdr) {
    int ret;
...
int esdec_enlarge_output_port(
    ...
    do {
        if (port->mem_size > 0) {
            ret = esdec_allocate_more_output_memorys(codec, port, dec_inst, dwl_inst, dev_mem_fd, buf_num);
            if (ret == FAILURE) {
                break;
            }
...
static int esdec_output_buffer_fd_split(
    ...
    if (pp_count == ES_VID_DEC_MAX_OUT_COUNT) {
        ret = ESDecGetDmaFdSplit(dwl_inst, dec_inst, dev_mem_fd, dma_fd, memory->fd, ES_VID_DEC_MAX_OUT_COUNT);
    }
...
void esdec_reset_output_memorys(ESOutputPort *port) {
    ...
    for (int i = 0; i < port->mem_num; i++) {
        ...
        if (memory->state == OUTPUT_MEMORY_STATE_CONSUMED) {
            ...
            esdec_set_output_buffer_state(memory, OUTPUT_MEMORY_STATE_CONSUME_QUEUE);
            es_fifo_queue_push(port->consumed_queue, &buffer, sizeof(buffer));
        }

        log_info(NULL,
                 "reset memory: %d , vir_addr: %p, state: %s\n",
                 i,
                 memory->vir_addr,
                 esdec_str_output_state(memory->state));
    }
