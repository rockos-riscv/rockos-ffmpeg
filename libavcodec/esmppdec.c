...
#if LIBDRM_VERSION_NUMBER < 0x02046400L
...
static av_cold int mpp_decode_close(AVCodecContext *avctx) {
    ...
    ES_RETURN_VAL_IF_FAIL(r, AVERROR(EINVAL));
    esmpp_set_device(r->mctx);
    ...
    if (r->buf_mode == ESMPP_DEC_PURE_EXTERNAL) {
        av_log(NULL, AV_LOG_DEBUG, "close esmpp put buf_group: %p\n", r->buf_group);
        mpp_buffer_group_put(r->buf_group);
        r->buf_group = NULL;
    }

    if (r->last_pkt.size > 0) {
...
static int mpp_decode_set_config(AVCodecContext *avctx) {
    ...
    if (r->disable_ec) {
        mpp_dec_cfg_set_s32(dec_cfg, "disable_ec", r->disable_ec);
    }
    if (r->drop_err_frame) {
        mpp_dec_cfg_set_s32(dec_cfg, "drop_err_frame", r->drop_err_frame);
    }
    ...
    mpp_dec_cfg_set_s32(dec_cfg, "time_base_num", avctx->pkt_timebase.num);
    mpp_dec_cfg_set_s32(dec_cfg, "time_base_den", avctx->pkt_timebase.den);
    mpp_dec_cfg_set_s32(dec_cfg, "input_frame_rate", avctx->framerate.num / avctx->framerate.den);
...
static av_cold int mpp_decode_init(AVCodecContext *avctx) {
    ...
    if ((ret = esmpp_select_dev(r->mctx, r->device_idx, r->die_idx)) != MPP_OK) {
...
static int mpp_decode_set_buffer_group(AVCodecContext *avctx, enum AVPixelFormat pix_fmt, MppFramePtr mpp_frame) {
    ...
    mppfc = hwfc->hwctx;
    AVHWDeviceContext *hwdev = hwfc->device_ctx;
    AVESMPPDeviceContext *hwctx = hwdev->hwctx;
    hwctx->die_idx = r->die_idx;
    hwctx->device_idx = r->device_idx;
    if (r->buf_mode != ESMPP_DEC_PURE_EXTERNAL) {
...
static int mpp_buffer_export_frame(AVFrame *frame, MppFramePtr mpp_frame) {
    ...
    desc->fd[0] = mpp_buffer_get_fd_sys(mpp_buf);
...
static int mpp_decode_get_frame(AVCodecContext *avctx, AVFrame *frame, int timeout) {
    ...
    if (r->info_change = mpp_frame_get_info_change(mpp_frame)) {
        ...
        av_log(avctx, AV_LOG_INFO, "Noticed an info change\n");
        ...
        if (avctx->width != avctx->coded_width || avctx->height != avctx->coded_height) {
        ...
        if (avctx->pix_fmt == AV_PIX_FMT_DRM_PRIME) {
            ...
            AVFrame *dev_frame;
            dev_frame = av_frame_alloc();
            dev_frame->format = avctx->pix_fmt;
            dev_frame->width = tmp_frame->width;
            dev_frame->height = tmp_frame->height;
            ret = av_hwframe_map(dev_frame, tmp_frame, AV_HWFRAME_MAP_READ);
            if (ret < 0) {
                av_log(avctx, AV_LOG_ERROR, "av_hwframe_map failed: %d\n", ret);
            }
            ESMPPDecContext *r = avctx->priv_data;
            if ((ret = av_frame_get_buffer(frame, r->stride_align)) < 0) {
                av_log(avctx, AV_LOG_ERROR, "av_frame_get_buffer failed: %d\n", ret);
                goto exit;
            }
            esmpp_hwframe_transfer(frame, dev_frame);
            av_frame_free(&dev_frame);
#else
...
static int mpp_decode_send_eos(AVCodecContext *avctx) {
    ...
    if (ret != MPP_OK) {
        ...
        av_log(avctx, AV_LOG_INFO, "send eos success\n");
    }
    r->draining = 1;
...
static int mpp_decode_send_packet(AVCodecContext *avctx, AVPacket *pkt) {
    ...
    if ((avctx->codec_id == AV_CODEC_ID_H264 || avctx->codec_id == AV_CODEC_ID_HEVC) && r->first_packet) {
        if (avctx->extradata_size) {
            ...
            if ((ret = esmpp_put_packet(r->mctx, mpp_pkt)) != MPP_OK) {
                ...
                av_log(avctx, AV_LOG_TRACE, "Wrote extradata %d bytes to decoder\n", avctx->extradata_size);
            }
            mpp_packet_deinit(&mpp_pkt);
        }
        r->first_packet = 0;
    ...
    if (r->wait_key_frame) {
        if (!r->found_key_frame) {
            if (pkt->flags & AV_PKT_FLAG_KEY) {
                r->found_key_frame = 1;
                av_log(avctx, AV_LOG_INFO, "waiting key frame enabled, find key frame pakcet.\n");
            } else {
                av_log(avctx, AV_LOG_INFO, "waiting key frame enabled, drop a non-key frame packet.\n");
    ...
    if ((ret = esmpp_put_packet(r->mctx, mpp_pkt)) != MPP_OK) {
        ...
        av_log(avctx, AV_LOG_TRACE, "Wrote %d bytes to decoder\n", pkt->size);
    }
    mpp_packet_deinit(&mpp_pkt);
...
static int mpp_decode_receive_frame(AVCodecContext *avctx, AVFrame *frame) {
    ...
    /* drain remain frames */
    if (r->draining) {
        ret = mpp_decode_get_frame(avctx, frame, MPP_TIMEOUT_MAX);
        goto exit;
    }
