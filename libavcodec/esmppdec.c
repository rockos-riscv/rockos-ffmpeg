...
static int validate_dec_pixfmt(int bits_depth, MppFrameFormat mppfmt) {
    if (bits_depth != 10 && mppfmt == MPP_FMT_P010) {
        av_log(NULL, AV_LOG_ERROR, "input data %dbits and output pixfmt 10bits unsupported", bits_depth);
        return -1;
    }
    return 0;
}
...
static int mpp_decode_set_config(AVCodecContext *avctx) {
    ...
    if (mppfmt != MPP_FMT_BUTT) {
        if (validate_dec_pixfmt(avctx->bits_per_raw_sample, mppfmt) < 0) {
...
static int mpp_decode_export_frame(AVCodecContext *avctx, AVFrame *frame, MppFramePtr mpp_frame) {
    ...
    enum AVPixelFormat actual_pixfmt = AV_PIX_FMT_NONE;
