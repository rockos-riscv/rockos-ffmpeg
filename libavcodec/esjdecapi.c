...
static int esdec_thumb_mem_fd_split(
    void *dwl_inst, void *dec_inst, int dev_mem_fd, ESThumbOutputMemory *memory, struct DecConfig *dec_config) {
    int ret = SUCCESS;
    int dma_fd;
    int pp_count = 0;
...
static int esjdec_allocate_thumbnail_memory(ESThumbOutputMemory *thumb_mem,
    ...
    if (info.next_buf_size != 0) {
        ...
        esdec.inst = dec_inst;
        esdec_thumb_mem_fd_split(dwl_inst, &esdec, dev_mem_fd, &thumb_mem, dec_config);
...
static int esjdec_decode_and_get_next_picture(JDECContext *dec_ctx,
    ...
    do {
        ...
        if (dec_ctx->need_out_buf_for_thumb) {
            ret = esjdec_allocate_thumbnail_memory(
                &dec_ctx->thumb_mem, dec_ctx->dec_inst, dec_ctx->dev_mem_fd, dec_ctx->dwl_ref, &dec_ctx->jdec_config);
            if (ret == FAILURE) {
