...
static int encoder_set_venc(MppEncCfgPtr cfg, AVCodecContext *avctx) {
    ...
    if (avctx->codec_id == AV_CODEC_ID_H264 || avctx->codec_id == AV_CODEC_ID_HEVC) {
        ...
        CFG_SET_S32_IF_USER_SET(cfg, "venc:bit_depth", cxt->bitdepth, -1);
    }
...
static ESMPPEncFrame *esmpp_submit_frame(AVCodecContext *avctx, AVFrame *frame) {
    ...
    if (avctx->pix_fmt == AV_PIX_FMT_DRM_PRIME) {
        ...
        memcpy(vir_addr, frame->data[0], luma_size);  // cp luma
        if (frame->linesize[1] && frame->linesize[2]) {
            memcpy(vir_addr + luma_size, frame->data[1], chroma_size / 2);
            memcpy(vir_addr + luma_size + chroma_size / 2, frame->data[2], chroma_size / 2);
        } else if (frame->linesize[1]) {
            memcpy(vir_addr + luma_size, frame->data[1], chroma_size);
        }
        // av_log(avctx,
        //        AV_LOG_DEBUG,
        //        "linesize %d-%d-%d lumasize %u chromasize %u, pic_size %u\n",
        //        frame->linesize[1],
        //        frame->linesize[1],
        //        frame->linesize[1],
        //        luma_size,
...
static int esmpp_encode_init(AVCodecContext *avctx) {
    ...
    esmpp_get_picsize(avctx->pix_fmt,
                      ...
                      &chroma_size,
                      &pic_size);
    mpp_buffer_group_limit_config(cxt->frame_grp, pic_size, 10);
