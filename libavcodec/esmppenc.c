...
static void clear_unused_frame(ESMPPEncFrame *list, MppFramePtr mpp_frame) {
    if (!mpp_frame) {
        return;
    }
    while (list) {
        if (list->queued == 1 && list->mpp_frame == mpp_frame) {
...
static int encoder_set_venc(MppEncCfgPtr cfg, AVCodecContext *avctx) {
    ...
    ES_RETURN_VAL_IF_FAIL(avctx, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(avctx->priv_data, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(cfg, AVERROR(EINVAL));
    cxt = avctx->priv_data;

    CFG_SET_S32(cfg, "venc:input_buffer_num", cxt->async_frames);
    CFG_SET_S32(cfg, "venc:time_base_den", avctx->time_base.den);
    CFG_SET_S32(cfg, "venc:time_base_num", avctx->time_base.num);
    ...
    CFG_SET_S32(cfg, "venc:max_width", cxt->max_width);
    CFG_SET_S32(cfg, "venc:max_height", cxt->max_height);
    ...
    if (avctx->codec_id == AV_CODEC_ID_H264 || avctx->codec_id == AV_CODEC_ID_HEVC) {
        ...
        if (avctx->codec_id == AV_CODEC_ID_HEVC) {
            ...
            CFG_SET_S32(cfg, "venc:preset", cxt->preset);
        }
        if (cxt->bitdepth == -1) {
    ...
    if (avctx->codec_id == AV_CODEC_ID_H264 || avctx->codec_id == AV_CODEC_ID_HEVC) {
        ...
        switch (cxt->gop_mode) {
            ...
            case VENC_GOPMODE_DUALREF: {
                ...
                CFG_SET_S32(cfg, "dualp:sp_qp_delta", cxt->sb_qp_delta);
...
static unsigned int encoder_get_framerate(AVRational av_framerate) {
    ...
    av_log(NULL, AV_LOG_TRACE, "get_framerate num:%d den:%d rate:%u\n", av_framerate.num, av_framerate.den, framerate);
    return framerate;
}
...
static int encoder_set_venc_rc(MppEncCfgPtr cfg, AVCodecContext *avctx) {
    ...
    switch (rc_mode) {
        ...
        case VENC_RC_MODE_H265CBR: {
            ...
            CFG_SET_S32(cfg, "rc_adv:first_frame_start_qp", cxt->start_qp);
...
static int encoder_set_venc_protocal(MppEncCfgPtr cfg, AVCodecContext *avctx) {
    ...
    if (avctx->codec_id == AV_CODEC_ID_H264 || avctx->codec_id == AV_CODEC_ID_HEVC) {
        ...
        // vui
        mpp_enc_cfg_set_s32(cfg, "vui:vui_present_flag", cxt->insert_vui);
        mpp_enc_cfg_set_s32(cfg, "vui:sar_preset_flag", cxt->insert_sar_at_vui);
        mpp_enc_cfg_set_s32(cfg, "vui:video_signal_type_present_flag", cxt->insert_video_signal_type_at_vui);
...
static int esmpp_set_enc_cfg(AVCodecContext *avctx) {
    ...
    if (encoder_set_venc_protocal(cfg, avctx)) {
        av_log(avctx, AV_LOG_ERROR, "Failed to set_venc_protocal config\n");
        return -1;
    }
...
static int esmpp_send_frame(AVCodecContext *avctx, ESMPPEncFrame *mpp_enc_frame) {
    ...
    if ((ret = esmpp_put_frame(cxt->mctx, mpp_frame)) != MPP_OK) {
        int log_level = (ret == MPP_ERR_INPUT_FULL) ? AV_LOG_TRACE : AV_LOG_ERROR;
        ...
        av_log(avctx, AV_LOG_TRACE, "Wrote %ld bytes to encoder\n", mpp_frame ? mpp_frame_get_buf_size(mpp_frame) : 0);
    }
...
static int esmpp_get_packet_internal(AVCodecContext *avctx, AVPacket *packet, int timeout) {
...
#ifdef TRANSCODE_HOST
    packet->data = av_malloc(packet->size);
    mpp_buffer_memcpy(packet->data, mpp_packet_get_data(mpp_pkt), packet->size);
#else
    ...
    if (!mpp_meta) {
        ...
        clear_unused_frame(cxt->frame_list, mpp_frame);
    }
    ...
    if (cxt->dump_pkt_enable) {
        ...
        if (ret == ERR_TIMEOUT) {
            av_log(NULL, AV_LOG_DEBUG, "pkt dump timeout\n");
            esmpp_codec_dump_file_close(&cxt->dump_pkt_hnd);
            cxt->dump_pkt_enable = 0;
            av_log(NULL, AV_LOG_DEBUG, "closed dump packet handle\n");
        } else if (ret < 0) {
...
static ESMPPEncFrame *esmpp_submit_frame(AVCodecContext *avctx, AVFrame *frame) {
    ...
    ES_RETURN_VAL_IF_FAIL(avctx, NULL);
    ES_RETURN_VAL_IF_FAIL(avctx->priv_data, NULL);
    ...
    if (avctx->pix_fmt == AV_PIX_FMT_DRM_PRIME) {
        ...
        // cp luma
        mpp_buffer_memcpy(vir_addr, frame->data[0], luma_size);
        if (frame->linesize[1] && frame->linesize[2]) {
            mpp_buffer_memcpy((uint8_t *)vir_addr + luma_size, frame->data[1], chroma_size / 2);
            mpp_buffer_memcpy((uint8_t *)vir_addr + luma_size + chroma_size / 2, frame->data[2], chroma_size / 2);
        } else if (frame->linesize[1]) {
            mpp_buffer_memcpy((uint8_t *)vir_addr + luma_size, frame->data[1], chroma_size);
        }
    }
...
static int esmpp_init_roi_delta_qp(VENC_ROI_ATTR_S *roi_attr) {
    ES_RETURN_VAL_IF_FAIL(roi_attr, -1);

    for (int i = 0; i < ES_VENC_MAX_ROI_NUM; i++) {
        roi_attr[i].bEnable = FALSE;
        roi_attr[i].bAbsQP = FALSE;
        roi_attr[i].QP = 0;
        roi_attr[i].rect.x = 0;
        roi_attr[i].rect.y = 0;
        roi_attr[i].rect.width = 0;
        roi_attr[i].rect.height = 0;
    }

    return 0;
}

static int esmpp_setup_roi(AVCodecContext *avctx,
                           const AVFrame *frame,
                           ESMPPEncFrame *mpp_enc_frame,
                           AVFrameSideData *sd) {
    ESMPPEncContext *cxt;
    ...
    if (MPP_VIDEO_CodingMJPEG != coding_type && mpp_enc_frame && mpp_enc_frame->mpp_frame) {
        MppMetaPtr meta = NULL;
        meta = mpp_frame_get_meta(mpp_enc_frame->mpp_frame);
        if (meta) {
            if (!mpp_enc_frame->roi_attr) {
                mpp_enc_frame->roi_attr = calloc(ES_VENC_MAX_ROI_NUM, sizeof(VENC_ROI_ATTR_S));
            } else {
                esmpp_init_roi_delta_qp(mpp_enc_frame->roi_attr);
            }
            if (!sd) {
                if (cxt->roi_attr_pre) {
                    free(cxt->roi_attr_pre);
                    cxt->roi_attr_pre = NULL;
                    mpp_meta_set_ptr(meta, KEY_ROI_DATA, mpp_enc_frame->roi_attr);  // clear roi
                } else {
                    free(mpp_enc_frame->roi_attr);
                    mpp_enc_frame->roi_attr = NULL;
                }
                return 0;
            }
            ...
            nb_rois = nb_rois > ES_VENC_MAX_ROI_NUM ? ES_VENC_MAX_ROI_NUM : nb_rois;
            av_log(avctx, AV_LOG_DEBUG, "number of rois: %d.\n", nb_rois);
            ...
            for (int i = nb_rois - 1; i >= 0; i--) {
                ...
                mpp_enc_frame->roi_attr[i].bEnable = TRUE;
                mpp_enc_frame->roi_attr[i].bAbsQP = FALSE;
                mpp_enc_frame->roi_attr[i].QP = roi->qoffset.num * H26X_MAX_QP / roi->qoffset.den;
                mpp_enc_frame->roi_attr[i].QP = mpp_enc_frame->roi_attr[i].QP == -1 * H26X_MAX_QP
                                                    ? -1 * (H26X_MAX_QP - 1)
                                                    : mpp_enc_frame->roi_attr[i].QP;
                mpp_enc_frame->roi_attr[i].rect.x = FFMIN(frame->width, roi->left);
                mpp_enc_frame->roi_attr[i].rect.y = FFMIN(frame->height, roi->top);
                mpp_enc_frame->roi_attr[i].rect.width =
                    FFMIN(frame->width, roi->right) - mpp_enc_frame->roi_attr[i].rect.x;
                mpp_enc_frame->roi_attr[i].rect.height =
                    FFMIN(frame->height, roi->bottom) - mpp_enc_frame->roi_attr[i].rect.y;
            }

            if (cxt->roi_attr_pre
                && !memcmp(cxt->roi_attr_pre, mpp_enc_frame->roi_attr, sizeof(*(mpp_enc_frame->roi_attr)))) {
                free(mpp_enc_frame->roi_attr);
                mpp_enc_frame->roi_attr = NULL;
                return 0;  // same roi attribution, do nothing.
            }

            if (!cxt->roi_attr_pre) {
                cxt->roi_attr_pre = calloc(ES_VENC_MAX_ROI_NUM, sizeof(VENC_ROI_ATTR_S));
            }
            memcpy(cxt->roi_attr_pre, mpp_enc_frame->roi_attr, sizeof(mpp_enc_frame->roi_attr));
            mpp_meta_set_ptr(meta, KEY_ROI_DATA, mpp_enc_frame->roi_attr);
            av_log(avctx, AV_LOG_DEBUG, "copy roi attr finished. rois num:%d\n", nb_rois);
        }
    }
...
static int esmpp_encode_frame(AVCodecContext *avctx, AVPacket *packet, const AVFrame *frame, int *got_packet) {
    ...
    ES_RETURN_VAL_IF_FAIL(avctx, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(packet, AVERROR(EINVAL));

    cxt = avctx->priv_data;

    if (frame && (frame->flags & AV_FRAME_FLAG_DISCARD)) {

        av_log(avctx, AV_LOG_DEBUG, "esmpp_encoder: discard frame pts=%" PRId64 "\n",
               frame->pts);
    ...
    if (frame) {
        ...
        ret = esmpp_setup_roi(avctx, frame, mpp_enc_frame, sd);
        if (ret < 0) {
            return ret;
        }
    }
...
send_frm:
    ...
    av_log(avctx, AV_LOG_TRACE, "send_frame ret:%d, sent frm cnt:%d\n", ret, cxt->sent_frm_cnt);
...
get_pkt:
    ...
    if (ret == AVERROR(EAGAIN)) {
        if (is_hevc_still_pic_profile(avctx->codec_id, cxt->profile)) {
            if (cxt->sent_frm_cnt == 1 && cxt->got_pkt_cnt < 1) {
                av_log(avctx, AV_LOG_DEBUG, "still pic frame is %p, ret %d\n", frame, ret);
                av_usleep(10 * 1000);
                goto get_pkt;
            }
        } else if (!frame) {  // sent all the frame we must get left packet
            av_log(avctx, AV_LOG_DEBUG, "frame is %p, ret %d\n", frame, ret);
            av_usleep(10 * 1000);
            goto get_pkt;
    ...
    if (ret == AVERROR_EOF || ret == AVERROR(EAGAIN)) {
        *got_packet = 0;
        ...
        av_log(avctx, AV_LOG_TRACE, "get_packet len %d ret:%d\n", packet->size, ret);
        return ret;
    } else {
        av_log(avctx,
               AV_LOG_TRACE,
...
static int esmpp_encode_get_header_packet(AVCodecContext *avctx, MppPacketPtr *mpp_pkt) {
    ...
    ES_RETURN_VAL_IF_FAIL(avctx, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(mpp_pkt, AVERROR(EINVAL));
    ...
    if (avctx->codec_id == AV_CODEC_ID_H264 || avctx->codec_id == AV_CODEC_ID_HEVC) {
        ...
        *mpp_pkt = tmp_pkt;
        ...
        mpp_buffer_memcpy(avctx->extradata, pkt_pos, pkt_len);
...
static int handle_mdcv(AVCodecContext *avctx, const AVMasteringDisplayMetadata *mdcv, MppEncCfgPtr cfg) {
    ES_RETURN_VAL_IF_FAIL(avctx, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(mdcv, AVERROR(EINVAL));
...
static int handle_side_data(AVCodecContext *avctx, ESMPPEncContext *params) {
    ES_RETURN_VAL_IF_FAIL(avctx, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(params, AVERROR(EINVAL));
...
static int esmpp_encode_init(AVCodecContext *avctx) {
    ...
    MppPacketPtr mpp_pkt = NULL;
    ES_S32 buf_type = MPP_BUFFER_TYPE_DMA_HEAP;
    int ret = 0;
    ...
    if (avctx->hw_device_ctx) {
        ...
        ES_RETURN_VAL_IF_FAIL(hwdev, AVERROR(EINVAL));
        hwctx = hwdev->hwctx;
        ES_RETURN_VAL_IF_FAIL(hwctx, AVERROR(EINVAL));

        cxt->die_idx = hwctx->die_idx;
        cxt->device_idx = hwctx->device_idx;
        av_log(avctx, AV_LOG_INFO, "%s set device: %d, die_idx: %d.\n", __func__, cxt->device_idx, cxt->die_idx);
    }

    buf_type = mpp_buffer_get_type(buf_type, cxt->die_idx);
    ret = mpp_buffer_group_get_internal(&cxt->frame_grp, buf_type);
    if (ret) {
    ...
    cxt->pix_fmt = pix_fmt;
    if (avctx->max_b_frames >= 1 || avctx->max_b_frames <= 3) {
        cxt->b_frm_num = avctx->max_b_frames;
    } else {
        cxt->b_frm_num = 2;
        av_log(avctx, AV_LOG_WARNING, "Unsupported max_b_frames: %d, correct it as 2\n", avctx->max_b_frames);
    }
    ...
    if ((ret = esmpp_select_dev(cxt->mctx, cxt->device_idx, cxt->die_idx)) != MPP_OK) {
        av_log(avctx, AV_LOG_ERROR, "Failed to select MPP dev: %d\n", ret);
        ret = AVERROR_EXTERNAL;
        goto fail;
    }
    ...
    if(cxt->async_frames == -1){
        cxt->async_frames = ASYNC_FRAMES;
    }
