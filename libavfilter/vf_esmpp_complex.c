...
static int complex_query_formats(AVFilterContext *ctx) {
    ...
    ES_RETURN_VAL_IF_FAIL(ctx->priv, AVERROR(EINVAL));
...
static av_cold void uninit(AVFilterContext *ctx) {
    ...
    ES_RETURN_IF_FAIL(ctx);
    ES_RETURN_IF_FAIL(ctx->priv);

    s = (MppFilterContext *)ctx->priv;

    esmpp_set_device(s->mctx);
...
static int parse_rect(const char *rect_cmd, RECT_S *rect) {
    ES_RETURN_VAL_IF_FAIL(rect_cmd, FAILURE);
...
static int parse_rotation(const char *rotation_cmd, ROTATION_E *rotation) {
    ES_RETURN_VAL_IF_FAIL(rotation_cmd, FAILURE);
    ES_RETURN_VAL_IF_FAIL(rotation, FAILURE);
...
static int parse_blend_mode(int blend_mode_cmd, int *mode) {
    ES_RETURN_VAL_IF_FAIL(mode, FAILURE);
...
static int esmpp_complex_filter_frame(int input_nb, AVFilterLink **link, AVFrame **in) {
    AVFilterLink *link_src = NULL;
    AVFrame *in_src = NULL, *in_dst = NULL;
    AVFilterContext *ctx = NULL;
    MppFilterContext *s = NULL;
    AVFilterLink *outlink;
    AVFilterLink *outlink_snapshot = NULL;
    AVFrame *out = NULL;
    ES_BOOL bneed_snapshot = ES_FALSE;
    int ret = SUCCESS;
    ...
    ES_RETURN_VAL_IF_FAIL(link, FAILURE);
    ...
    if (s->blend_mode_set != -1) {
        ...
        if (check_rect_if_valid(
                &s->disp_rect, mpp_frame_get_width(dst_mpp_frame), mpp_frame_get_height(dst_mpp_frame), s->out_fmt)) {
            av_log(ctx, AV_LOG_ERROR, "vf[esmpp_complex] check discp rect failed.\n");
            ret = FAILURE;
            goto exit3;
        }
        mpp_ret = esmpp_process_frame_sync(s->mctx,
                                           src_mpp_frame,
                                           0,
                                           dst_mpp_frame,
                                           &s->src_rect,
        ...
        mpp_ret = esmpp_process_frame_sync(s->mctx,
                                           ...
                                           ES_NULL,
                                           (void *)&s->nor_params,
                                           TDE_USAGE_NORMALIZATION);
    } else {
        mpp_ret = esmpp_process_frame_sync(
            s->mctx, src_mpp_frame, 0, dst_mpp_frame, &s->src_rect, &s->dst_rect, ES_NULL, ES_NULL, TDE_USAGE_COMPLEX);
    }
    if (mpp_ret != MPP_OK) {
        ret = FAILURE;
        goto exit3;
...
exit1:
    ...
    if (ret == SUCCESS) {
        // do snapshot
        bneed_snapshot = esmpp_check_snapshot(s->snapshot_interval, s->out_cnt + 1, (ctx->nb_outputs > 1));
        if (bneed_snapshot) {
            outlink_snapshot = ctx->outputs[1];

            if (esmpp_snapshot_send_frame(outlink_snapshot, out) >= 0) {
                s->snapshot_cnt++;
                av_log(ctx,
                       AV_LOG_DEBUG,
                       "snapshot send success. outframe cnt:%d, snapshot cnt:%d\n",
                       s->out_cnt + 1,
                       s->snapshot_cnt);
#ifdef TRANSCODE_INFO_REPORT
                ...
                av_log(ctx,
                       AV_LOG_ERROR,
                       "snapshot send failed. ret:%d, outframe cnt:%d, snapshot send cnt:%d\n",
                       ret,
                       s->out_cnt + 1,
                       s->snapshot_cnt);
            }
...
static int process_frame(FFFrameSync *fs) {
    ...
    ES_RETURN_VAL_IF_FAIL(fs, FAILURE);
    ES_RETURN_VAL_IF_FAIL(fs->opaque, FAILURE);
...
static int complex_init_output_bufgroup(AVFilterLink *outlink) {
    ...
    ES_RETURN_VAL_IF_FAIL(outlink->src, FAILURE);
...
static int init_framesync(AVFilterContext *avctx) {
    ...
    ES_RETURN_VAL_IF_FAIL(avctx, FAILURE);
    ES_RETURN_VAL_IF_FAIL(avctx->priv, FAILURE);

    s = (MppFilterContext *)avctx->priv;
...
static av_cold int complex_config_props(AVFilterLink *outlink) {
    ...
    if (output == 0) {
        ...
        av_log(ctx,
               ...
               outlink->w,
               outlink->h,
               in_frame_rate / s->snapshot_interval);
        outlink->frame_rate = av_d2q(in_frame_rate / s->snapshot_interval, 1000);
    }
    ...
    if (check_rect_if_valid(&s->src_rect, inlink_main->w, inlink_main->h, s->in_fmt)) {
        av_log(ctx, AV_LOG_ERROR, "vf[esmpp_complex] check src rect failed.\n");
        return FAILURE;
    }
    if (check_rect_if_valid(&s->dst_rect, outlink->w, outlink->h, s->out_fmt)) {
        av_log(ctx, AV_LOG_ERROR, "vf[esmpp_complex] check dst rect failed.\n");
        return FAILURE;
    }

    if (parse_normalization(s)) {
        av_log(ctx, AV_LOG_ERROR, "vf[esmpp_complex] set normalization failed.\n");
        return FAILURE;
    }
...
static const AVOption options[] = {
    ...
    {"nor_mode",
     ...
     FLAGS,
     "nor_mode"},
    {"min_max", "Set the normalization meanValue", 0, AV_OPT_TYPE_CONST, {.i64 = 0}, 0, INT_MAX, FLAGS, "nor_mode"},
    {"z_score", "Set the normalization z_score", 0, AV_OPT_TYPE_CONST, {.i64 = 1}, 0, INT_MAX, FLAGS, "nor_mode"},
    {"nor_mm_r",
     ...
     0,
     FLAGS},
    {"nor_min", "set normalization minValue, such as:R/G/B", OFFSET(nor_min), AV_OPT_TYPE_STRING, {.str = NULL}, 0, 0, FLAGS},
    {"nor_mean",
    ...
    {"nor_std_r",
     ...
     0,
     FLAGS},
    {"nor_step_r", "set normalization stepReciprocal", OFFSET(nor_step_r), AV_OPT_TYPE_INT, {.i64 = 0}, 0, INT_MAX, FLAGS},
    {NULL},
};
