...
static int esmpp_stack_set_mpp_frame(MppFramePtr mpp_frame,
    ...
    ES_RETURN_VAL_IF_FAIL(mpp_frame, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(in, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(stride, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(offset, AVERROR(EINVAL));
...
static int esmpp_stack_process_logo(AVFilterContext *avctx,
    ...
    ES_RETURN_VAL_IF_FAIL(logo_frame, FAILURE);
    ES_RETURN_VAL_IF_FAIL(dst_mpp_frame, FAILURE);
    ...
    if (check_rect_if_valid(
            &logo_rect, mpp_frame_get_width(dst_mpp_frame), mpp_frame_get_height(dst_mpp_frame), esctx->out_sw_fmt)) {
        av_log(esctx, AV_LOG_ERROR, "stack check logo rect failed.\n");
        ret = FAILURE;
        goto exit;
    }
...
static int esmpp_stack_filter_frame(int input_nb, AVFilterContext *avctx, MppInputParams *inParams, AVFrame *oframe) {
    ...
    ES_RETURN_VAL_IF_FAIL(avctx, FAILURE);
    ES_RETURN_VAL_IF_FAIL(avctx->priv, FAILURE);
    ES_RETURN_VAL_IF_FAIL(inParams, FAILURE);
    ES_RETURN_VAL_IF_FAIL(oframe, FAILURE);
    ...
    if (inParams->fillcolor_enable) {
        ...
        mpp_ret = esmpp_process_frame_sync(
            esctx->mctx, ES_NULL, 1, dst_mpp_frame, ES_NULL, &rect_info, ES_NULL, ES_NULL, TDE_USAGE_COLOR_FILL);
        if (mpp_ret != MPP_OK) {
            ret = FAILURE;
...
static int esmpp_sort_frame_by_layer(AVFilterContext *avctx, RECT_S *src_rects, RECT_S *dst_rects, AVFrame **in) {
    ...
    ES_RETURN_VAL_IF_FAIL(avctx, FAILURE);
    ES_RETURN_VAL_IF_FAIL(avctx->priv, FAILURE);
    ES_RETURN_VAL_IF_FAIL(src_rects, FAILURE);
    ES_RETURN_VAL_IF_FAIL(dst_rects, FAILURE);
    ES_RETURN_VAL_IF_FAIL(in, FAILURE);

    sctx = avctx->priv;
    next_layer = sctx->regions[0].layer;
...
static int process_frame(FFFrameSync *fs) {
    ...
    // do snapshot
    bneed_snap = esmpp_check_snapshot(ctx->snap_interval, ctx->out_cnt + 1, (avctx->nb_outputs > 1));
    if (bneed_snap) {
        outlink_snap = avctx->outputs[1];
        ret = esmpp_snapshot_send_frame(outlink_snap, oframe);
        if (ret >= 0) {
            ctx->snapshot_cnt++;
            av_log(avctx,
                   AV_LOG_DEBUG,
                   "stack send success. outframe cnt:%d, snapshot cnt:%d\n",
                   ctx->out_cnt + 1,
                   ctx->snapshot_cnt);
#ifdef TRANSCODE_INFO_REPORT
            ...
            av_log(avctx,
                   AV_LOG_ERROR,
                   "stack snapshot send failed. ret:%d, outframe cnt:%d, snapshot cnt:%d\n",
                   ret,
                   ctx->out_cnt + 1,
                   ctx->snapshot_cnt);
        }
...
static int config_output(AVFilterLink *outlink) {
    ...
    ES_RETURN_VAL_IF_FAIL(outlink, AVERROR(EINVAL));
    ES_RETURN_VAL_IF_FAIL(outlink->src, AVERROR(EINVAL));
    ...
    for (int i = 0; i < sctx->nb_inputs; i++) {
        RECT_S rect = {sctx->regions[i].x, sctx->regions[i].y, sctx->regions[i].width, sctx->regions[i].height};
        if (check_rect_if_valid(&rect, outlink->w, outlink->h, esctx->out_sw_fmt)) {
            av_log(ctx, AV_LOG_ERROR, "stack config_output check region failed.\n");
            return FAILURE;
        }
        av_log(ctx,
               AV_LOG_DEBUG,
...
static int esmpp_stack_init_parse(AVFilterContext *avctx) {
    ...
    ES_RETURN_VAL_IF_FAIL(avctx->priv, AVERROR(EINVAL));
