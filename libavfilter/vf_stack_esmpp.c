...
static int esmpp_stack_filter_frame(int input_nb, AVFilterContext *avctx, MppInputParams *inParams, AVFrame *oframe) {
    ...
    // handle dst mpp frame
    {
        ...
        if (esctx->out_hw_frame_ref && oframe->buf[0]) {
...
static int config_output(AVFilterLink *outlink) {
    ...
    if (inlink0->format == AV_PIX_FMT_DRM_PRIME && inlink0->hw_frames_ctx) {
        AVHWFramesContext *in_hw_frames_ctx = (AVHWFramesContext *)inlink0->hw_frames_ctx->data;
        AVHWDeviceContext *hwdev = in_hw_frames_ctx->device_ctx;
        AVESMPPDeviceContext *hwctx = hwdev->hwctx;
        esctx->die_idx = hwctx->die_idx;
        esctx->device_idx = hwctx->device_idx;
        av_log(ctx, AV_LOG_INFO, "%s set device: %d, die_idx: %d.\n", __func__, esctx->device_idx, esctx->die_idx);
