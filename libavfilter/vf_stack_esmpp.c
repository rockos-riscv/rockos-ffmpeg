...
static int esmpp_stack_set_mpp_frame(
    MppFramePtr mpp_frame, const AVFrame *in, ROTATION_E rotation, ES_S32 global_alpha, enum AVPixelFormat fmt) {
...
static int esmpp_stack_filter_frame(int input_nb, AVFilterContext *avctx, MppInputParams *inParams, AVFrame *oframe) {
    ...
    for (ES_S32 i = 0; i < input_nb; i++) {
        ...
        esmpp_stack_set_mpp_frame(mpp_frame, in_src[i], 0, -1, esctx->in_sw_fmt[i]);
        if (esctx->format == AV_PIX_FMT_DRM_PRIME && in_src[i]->hw_frames_ctx) {
            ...
            // memcpy(mpp_buffer_get_ptr(mpp_buf), in_src[i]->buf[0]->data, in_frame_size[i]);
    ...
    // handle dst mpp frame
    {
        ...
        mpp_frame_set_fmt(dst_mpp_frame, ff_fmt_to_mpp_fmt(esctx->out_sw_fmt));
        mpp_frame_set_rotation(dst_mpp_frame, 0);
        mpp_frame_set_global_alpha(dst_mpp_frame, -1);
        mpp_frame_set_hor_stride(dst_mpp_frame, get_alignment_by_format(esctx->out_sw_fmt));
